...
namespace AwokeKnowing.GnuplotCSharp
{
    ...
    {
        private readonly Process ExtPro;
        private readonly StreamWriter GnupStWr;
        private readonly List<StoredPlot> PlotBuffer = new List<StoredPlot>();
        private readonly List<StoredPlot> SPlotBuffer = new List<StoredPlot>();
        private bool ReplotWithSplot;

        public bool Hold { get; private set; }

        public GnuPlot(string gnuPlotFolder = @"C:\Program Files (x86)\gnuplot\bin")
        {
            ExtPro = new Process();
            ExtPro.StartInfo.FileName = Path.Combine(gnuPlotFolder, "gnuplot.exe");
    ...
    public class StoredPlot
    {
        ...
        public double[,] ZZ = new double[0,0];
        public int YSize;
